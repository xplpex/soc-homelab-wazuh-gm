...
![Servidor Ubuntu rodando](1InicioUbuntu.PNG)

Durante o provisionamento da stack pesada do SIEM, enfrentei um cenário clássico de **Incident Response de Infraestrutura**: esgotamento de disco.
Para resolver a indisponibilidade, foi necessário atuar diretamente no sistema de arquivos do Linux, expandindo fisicamente o disco e realizando o resize da partição LVM (`lvextend` e `resize2fs`) sem perda de dados.

*Subindo a stack do SOC via Docker Compose:*
![Docker Compose Up](2Updocker.PNG)
...
![Wazuh Agent Status](4AgentWazuhrodando.PNG)
*Painel Geral do SIEM:*
![Wazuh Dashboard](3wazuhrodando.PNG)
...
Foram executados:
1.  **Ataques manuais de SQL Injection:** Utilizando `curl` para injetar payloads maliciosos na URL.
2.  **Scanner de Vulnerabilidades:** Utilização do `Nikto` para gerar volume massivo de requisições agressivas e forçar o motor de detecção.
...
![Visão do Kali Linux e retorno 403](5ataque.jpg)
...
![Pico de ataques e regra Nível 7](6Socvendoatack.PNG)
...
* Interceptar o ID original do log do ModSecurity.
* Elevar a severidade para **Nível 12 (Critical)**.
* Adicionar uma descrição precisa do incidente.
* Mapear a detecção para a tática **T1190 (Exploit Public-Facing Application)** do framework **MITRE ATT&CK**.

*Tuning e criação da nova regra no SIEM:*
![Criação de Regra no Wazuh](7MelhoradeRegrea.PNG)
...
![Alerta Crítico Nível 12 gerado no painel](8AlertaPosRegra.PNG)
